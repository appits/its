...
class PurchaseBook(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if sum_vat_additional_base != 0:
            sum_ali_gene_addi =  sum_vat_additional_base - sum_vat_additional_base_importaciones
            sum_vat_additional_base = sum_vat_additional_base - sum_vat_additional_base_importaciones
        else:
            sum_ali_gene_addi = sum_vat_additional_base

        if sum_vat_additional_tax != 0:
            sum_ali_gene_addi_credit = sum_vat_additional_tax - sum_vat_additional_tax_importaciones
            sum_vat_additional_tax = sum_vat_additional_tax - sum_vat_additional_tax_importaciones
        else:
            sum_ali_gene_addi_credit = sum_vat_additional_tax

        if sum_vat_general_base != 0:
            sum_vat_general_base = sum_vat_general_base - suma_base_general_importaciones
            sum_vat_general_tax =  sum_vat_general_tax - sum_base_general_tax_importaciones
        if sum_vat_reduced_base != 0:
            sum_vat_reduced_base = sum_vat_reduced_base - sum_vat_reduced_base_importaciones
            sum_vat_reduced_tax = sum_vat_reduced_tax - sum_vat_reduced_tax_importaciones
